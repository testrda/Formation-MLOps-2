## nb : on pourra lancer l'appli grâce à la commande bash `FLASK_APP=inference.py python -m flask run`
...
## on ne devrait pas importer notre package de modélisation depuis le wheel du dossier plutôt ?
## pour être plus proche de l'environnement cible ?
from formation_indus_ds_avancee.feature_engineering import prepare_features
from formation_indus_ds_avancee.train_and_predict import predict

app = Flask(__name__)

## définition des X par défaut, qu'on updatera selon les args de la requête get reçue du client
default_args = {"Wind_turbine_name": "R80721", "Date_time": "2017-02-08T08:00:00+01:00", "Ba_avg": 44.99, "Ba_min": 44.99,
...
def predict_endpoint():
    new_arg = dict(request.args) ## TODO : voir si pas possible de faire mieux, si ça se trouve les multiDictMachin ont des propriétés intéressantes et peuvent s'updater également

    ## conversion des arguments en floats
    new_arg = {k:float(v) for k,v in new_arg.items()} ## TODO : ne convertir que les arguments qui doivent être reçus comme des floats, et non ceux qui doivent rester sous la forme de tests

    #import pdb;pdb.set_trace()

    args_to_inference = dict(default_args) ## copie des arguments par défaut
    args_to_inference.update(new_arg) ## update avec les arguments reçus
    received_data_df = pd.DataFrame(args_to_inference,index=[0]) ## transformation en dataframe

    ## inference (preparation + predict)
